...
## 简介
本文件为应用的数据模型提供全面的类型定义说明，覆盖 ChatGroup、Thread（线程摘要）、MessageItem、TurnRecord、Approval、ModelProfile、AppSettings 等核心实体。文档详细说明各实体的字段定义、数据类型与业务含义，解释实体间关系映射与数据转换逻辑，记录数据验证规则、默认值处理与必填字段约束，并提供 TypeScript 接口使用示例、序列化/反序列化策略以及版本兼容性与向后兼容性策略。
...
- [src/agent/database.ts:1-1427](file://src/agent/database.ts#L1-L1427)
...
## 核心组件
本节概述核心实体及其职责：
- ChatGroup：聊天分组，用于组织 Thread。
- ThreadSummary：线程摘要，包含标题、状态、关联的模型配置等。
...
- 业务含义：用于对 Thread 进行分组管理，支持软删除（数据库层面有 deleted_at）。
- 默认值/必填：name 必填且需去空白；时间戳由系统生成。
- 关系：一个 ChatGroup 可包含多个 Thread。

**章节来源**
- [src/shared/domain.ts:7-12](file://src/shared/domain.ts#L7-L12)
...
- 字段
  ...
  - createdAt: string
  - updatedAt: string
- 业务含义：线程是会话的基本单元，承载 Turn 与 Items。
- 默认值/必填：title 必填；status 初始为 ready；modelProfileId 可为空。
- 关系：属于 ChatGroup；包含多条 TurnRecord 与 Item。
...
**章节来源**
- [src/shared/domain.ts:294-305](file://src/shared/domain.ts#L294-L305)
...
**章节来源**
- [src/shared/domain.ts:76-85](file://src/shared/domain.ts#L76-L85)
...
**章节来源**
- [src/shared/domain.ts:1-115](file://src/shared/domain.ts#L1-L115)
...
```mermaid
erDiagram
CHAT_GROUP ||--o{ THREAD_SUMMARY : contains
...
### 数据验证规则与默认值
- 必填字段：ChatGroup.name、ThreadSummary.title、MessageItem.text、TurnRecord.status、Approval.title/description/status、ModelProfile.name/model/baseUrl/provider。
- 默认值：AppSettings.theme=system、language=en-US、showModelMetrics=true、contextMessageLimit=20、reasoningDisplayMode=auto；TurnRecord.incomplete 初始为 true；ModelProfile.maxConcurrency/maxOutputTokens 经归一化。
...
- [src/shared/domain.ts:171-190](file://src/shared/domain.ts#L171-L190)

### TypeScript 接口使用示例
- 创建分组：调用 createGroup(name)，返回 ChatGroup。
- 创建线程：createThread(title, groupId?)，返回 ThreadSummary。
