...
- [chatContext.test.ts](file://tests/chatContext.test.ts)
...
## 更新摘要
**所做更改**
- 新增本地上下文压缩系统章节，详细说明自动历史消息压缩机制
- 更新消息构建流程，集成新的上下文压缩功能
- 添加历史消息压缩算法和配置参数说明
- 增强压缩质量评估部分，包含本地压缩效果分析
- 更新监控方法，增加本地压缩指标追踪
...
**最新更新**：新增了本地上下文压缩系统，在消息超过6,000字符时自动压缩历史消息，保留前2,800和后2,800字符，并在中间插入省略标记，确保当前用户请求保持完整。

## 项目结构
上下文压缩能力集中在模型调用层和本地消息处理层，围绕一次对话轮次（turn）的请求构建、流式响应处理与错误恢复展开：
- **本地消息压缩**：在聊天上下文构建阶段对超长历史消息进行预处理压缩
- **模型级压缩**：在模型提供者中完成初始请求与续写请求的消息组装与压缩
...
B --> C["本地消息压缩<br/>compactHistoryMessageText"]
C --> D["模型提供者<br/>streamChatCompletion"]
D --> E["初始请求消息构建<br/>initialRequestMessages / compressedInitialRequestMessages"]
D --> F["续写请求消息构建<br/>continuationMessages / compressedContinuationMessages"]
D --> G["流式读取与指标合并<br/>readSSEDeltas / mergeStreamMetrics"]
D --> H["错误检测与重试<br/>isContextLimitFailure / isCompressedProviderFailure"]
I["模型能力与配置<br/>ModelCapabilities / RuntimeModelProfile"] --> D
J["共享领域模型<br/>ModelRunPhase / ModelRunMetrics"] --> D
```

**图表来源**
- [chatContext.ts:20-29](file://src/agent/chatContext.ts#L20-L29)
...
## 核心组件
- **本地上下文压缩**
  - 自动检测超过6,000字符的历史消息并进行压缩
  - 保留前2,800字符和后2,800字符，中间插入省略标记
  - 仅应用于历史消息，确保当前用户请求保持完整
- **上下文软限制与阈值**
...
**章节来源**
- [chatContext.ts:4-18](file://src/agent/chatContext.ts#L4-L18)
...
## 架构总览
下图展示一次对话轮次的完整流程，包括本地消息压缩、软限制判断、压缩决策、流式响应处理与错误重试。

```mermaid
sequenceDiagram
participant App as "调用方"
participant CC as "聊天上下文"
participant MP as "模型提供者"
participant Net as "网络/服务端"
App->>CC : buildChatMessages(history, limit)
CC->>CC : compactHistoryMessageText(历史消息)
CC-->>MP : 压缩后的消息列表
...
### 本地上下文压缩系统
**新增功能**：实现了智能的历史消息压缩机制，防止单个超长回答膨胀后续提示。

- **压缩触发条件**
  - 当历史消息文本长度超过6,000字符时自动触发压缩
  - 仅应用于历史消息，当前用户请求保持完整
- **压缩策略**
  - 保留消息开头2,800字符和结尾2,800字符
  - 在中间位置插入省略标记：'[… middle of this message omitted by local context compaction …]'
  - 使用换行符连接各部分，保持格式清晰
- **集成方式**
  - 在 `buildChatMessages` 函数中自动应用压缩
  - 通过索引判断区分当前消息和历史消息
  - 与消息数量限制配合工作，先裁剪消息数量再压缩内容

```mermaid
flowchart TD
Start(["开始"]) --> Check{"消息长度 > 6000?"}
Check -- 否 --> Keep["保持原样"]
Check -- 是 --> Compress["执行压缩"]
Compress --> Head["保留前2800字符"]
Head --> Notice["插入省略标记"]
Notice --> Tail["保留最后2800字符"]
Tail --> Join["用换行符连接"]
Join --> End(["结束"])
Keep --> End
```
...
- [modelProvider.ts:280-332](file://src/agent/modelProvider.ts#L280-332)
...
- [chatContext.test.ts:16-54](file://tests/chatContext.test.ts#L16-L54)
...
- **场景化参数建议**
  ...
  - **新增**：合理设置 `contextMessageLimit` 控制历史消息数量，配合本地压缩效果更佳
- **效果对比维度**
...
- **监控点**
  ...
  - **新增**：本地压缩触发次数、压缩率、消息长度变化
- **基准测试建议**
  - 构造不同长度的历史与用户输入，记录压缩触发率与重试次数
  - 对比开启/关闭长上下文能力下的表现差异
  - 验证错误映射与边界情况（如 length 终止、超时、离线）
  - **新增**：测试本地压缩在不同消息长度下的效果
- **测试覆盖**
  - **新增**：完整的单元测试覆盖本地压缩功能，包括边界情况和异常处理
...
- **模块耦合**
  ...
  - worker 通过 chatContext 获取压缩后的消息列表
...
- [chatContext.ts:20-29](file://src/agent/chatContext.ts#L20-L29)
- [modelProvider.ts:55-197](file://src/agent/modelProvider.ts#L55-L197)
- [worker.ts:416-419](file://src/agent/worker.ts#L416-L419)
- [domain.ts:104-257](file://src/shared/domain.ts#L104-L257)
- [modelCapabilities.ts:14-24](file://src/agent/modelCapabilities.ts#L14-L24)
...
## 结论
该上下文压缩系统通过精确的令牌估算、合理的预算分配与多级压缩策略，在保证语义完整性的前提下有效应对长对话与复杂任务。**新增的本地上下文压缩系统**进一步增强了处理能力，通过自动压缩超长历史消息，防止单个回答膨胀影响后续对话。结合阶段可见性与指标监控，可在不同场景下灵活调优，平衡延迟、吞吐与质量。建议在工程实践中持续收集指标与反馈，迭代优化预算比例与压缩阈值。

## 附录
- **关键常量与类型**
  - 压缩阈值、最小续写段令牌、近似字符/令牌比、默认与长上下文窗口大小
  - 运行阶段、指标字段、能力声明等类型定义
  - **新增**：本地压缩常量（HISTORY_MESSAGE_MAX_CHARS、HISTORY_MESSAGE_HEAD_CHARS、HISTORY_MESSAGE_TAIL_CHARS）
- **参考实现路径**
  - 消息构建与压缩函数、流式读取与指标合并、错误检测与重试逻辑
  - **新增**：本地消息压缩函数和集成点
- **测试用例**
  - **新增**：完整的本地压缩功能测试，覆盖各种边界情况和异常处理
