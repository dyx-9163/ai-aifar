...
## 架构总览
下图展示了从调用到模型服务的端到端流程，包括流式分片处理、阶段推进、指标聚合与错误处理。
...
Finish -- length --> Compress{"是否需要压缩/续写?"}
...
- 测试覆盖：
  - modelProvider.test.ts 覆盖错误脱敏、上下文限制、超时、连接诊断等。
...
## 结论
本项目通过统一的模型接口抽象了不同后端（OpenAI 兼容 API 与本地 Qwen）的差异，提供了健壮的流式处理、上下文压缩与续写、严格的超时与错误映射、以及完善的连接测试与能力检测。借助标准化的能力声明与推理协议校验，系统可在多模型间灵活切换并保持一致的体验。
