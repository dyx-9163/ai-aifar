# 数据库层

<cite>
**本文引用的文件**
- [src/agent/database.ts](file://src/agent/database.ts)
- [tests/database.test.ts](file://tests/database.test.ts)
- [src/shared/domain.ts](file://src/shared/domain.ts)
</cite>

## 更新摘要
**所做更改**
- 移除了所有聊天组相关方法（createGroup, deleteGroup, defaultGroupId, ensureDefaultGroup）
- createThread方法现在接受可选的workspaceId参数并包含工作区验证
- 新增setThreadPinned和bindThreadWorkspace方法
- 更新了线程与工作区的绑定关系管理
- 简化了数据模型，移除了分组概念

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与优化](#性能与优化)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录：CRUD 与高级查询示例](#附录crud-与高级查询示例)

## 简介
本章节面向 SQLite 数据层的综合文档，覆盖表结构设计、关系映射、索引与查询优化、迁移策略（版本管理、向后兼容、回滚）、数据模型定义（实体关系、验证规则、约束）、CRUD 与高级查询模式、备份恢复策略与性能监控方法。内容基于仓库中的实现与测试用例进行归纳与说明。

**更新** 数据库层已重构以支持新架构，移除了所有聊天组相关方法，增强了线程与工作区的绑定关系管理。

## 项目结构
- 数据库实现位于 agent 层，通过 Node.js 内置的 sqlite 模块以同步方式访问本地 SQLite 文件。
- 领域类型集中在 shared/domain.ts，用于描述会话、消息、推理项、工作区、模型配置等实体。
- 数据库行为由大量单元测试覆盖，确保迁移、流式合并、事务原子性等关键路径正确。

```mermaid
graph TB
A["应用调用方"] --> B["SqliteAppDatabase<br/>接口: AppDatabase"]
B --> C["SQLite 引擎<br/>node:sqlite (DatabaseSync)"]
B --> D["迁移系统<br/>schema_migrations + ensureColumn/applyMigration"]
B --> E["数据模型映射<br/>map* / parse* / normalize*"]
C --> F["持久化文件<br/>app.sqlite"]
```

图表来源
- [src/agent/database.ts:185-189](file://src/agent/database.ts#L185-L189)
- [src/agent/database.ts:834-986](file://src/agent/database.ts#L834-L986)
- [src/shared/domain.ts:1-200](file://src/shared/domain.ts#L1-L200)

章节来源
- [src/agent/database.ts:185-189](file://src/agent/database.ts#L185-L189)
- [src/shared/domain.ts:1-200](file://src/shared/domain.ts#L1-L200)

## 核心组件
- SqliteAppDatabase：对外暴露 AppDatabase 接口，封装所有读写操作、迁移、事务和快照聚合。
- 迁移系统：使用 schema_migrations 表记录已执行迁移；通过 ensureColumn 与 applyMigration 实现增量升级与幂等性。
- 数据映射：将行数据映射为领域对象，并在读取时做安全脱敏（如隐藏 API Key）。
- 流式合并：对同一逻辑流的 assistant 消息与 reasoning 片段进行合并，避免冗余存储。
- 启动修复：重启后中断的 turn 与 pending approvals 会被自动修复，保证状态一致性。

**更新** 新增了线程固定和工作区绑定功能，移除了聊天组相关的复杂关系。

章节来源
- [src/agent/database.ts:185-189](file://src/agent/database.ts#L185-L189)
- [src/agent/database.ts:834-986](file://src/agent/database.ts#L834-L986)
- [src/agent/database.ts:495-517](file://src/agent/database.ts#L495-L517)
- [src/agent/database.ts:1234-1291](file://src/agent/database.ts#L1234-L1291)

## 架构总览
下图展示从应用调用到 SQLite 的完整流程，包括事务边界、迁移检查、流式写入合并以及快照聚合。

```mermaid
sequenceDiagram
participant U as "调用方"
participant DB as "SqliteAppDatabase"
participant S as "SQLite 引擎"
U->>DB : createThread()/appendItem()/completeTurn()
DB->>S : BEGIN IMMEDIATE
DB->>S : INSERT/UPDATE/UPSERT
DB->>DB : insertOrMergeItem(流式合并)
DB->>S : COMMIT
U-->>DB : getSnapshot()
DB->>S : SELECT 多表 JOIN
DB-->>U : 聚合后的 AppSnapshot
```

图表来源
- [src/agent/database.ts:191-253](file://src/agent/database.ts#L191-L253)
- [src/agent/database.ts:255-291](file://src/agent/database.ts#L255-L291)
- [src/agent/database.ts:495-517](file://src/agent/database.ts#L495-L517)

## 详细组件分析

### 表结构与关系映射
- 核心表
  - threads：对话线程，关联 workspace_id 与 model_profile_id，软删除，支持 pinned 标记。
  - turns：回合记录，关联 thread_id，包含生命周期状态、错误、指标等。
  - items：消息/推理/工具/变更等事件，按 thread_id 与可选 turn_id 组织，payload 为 JSON。
  - approvals：审批项，关联 thread_id 与 turn_id，支持 pending/approved/rejected。
  - settings：键值型设置，持久化 UI 与运行时偏好。
  - workspaces：工作区元信息，canonical_root_path 唯一。
  - file_checkpoints：文件变更检查点，支持撤销操作。
  - model_profiles：模型配置，包含能力、推理设置、并发与输出限制等。
  - schema_migrations：迁移版本记录。

**更新** 移除了 chat_groups 表的业务逻辑，threads 表现在直接关联 workspaces 而非 groups。

- 关系与约束
  - turns.thread_id -> threads.id（ON DELETE CASCADE）
  - items.thread_id -> threads.id（ON DELETE CASCADE）
  - items.turn_id -> turns.id（ON DELETE SET NULL）
  - approvals.thread_id -> threads.id（ON DELETE CASCADE）
  - approvals.turn_id -> turns.id（ON DELETE CASCADE）
  - file_checkpoints.workspace_id -> workspaces.id（ON DELETE CASCADE）
  - file_checkpoints.turn_id -> turns.id（ON DELETE CASCADE）
  - workspaces.canonical_root_path 唯一约束
  - 外键开启 PRAGMA foreign_keys = ON

- 索引与查询优化
  - 当前代码未显式创建额外索引；主要依赖主键与外键关系。
  - 常见查询通过 WHERE + ORDER BY 过滤与排序，适合中小规模数据。
  - 建议后续针对高频条件列添加索引（例如 threads.updated_at、turns.status、items.created_at），以提升列表与历史加载性能。

章节来源
- [src/agent/database.ts:834-986](file://src/agent/database.ts#L834-L986)
- [src/agent/database.ts:826-832](file://src/agent/database.ts#L826-L832)
- [src/agent/database.ts:191-253](file://src/agent/database.ts#L191-L253)

### 数据迁移策略
- 版本管理
  - 使用 schema_migrations 表记录每个迁移版本号与执行时间。
  - 初始化阶段创建基础表并插入默认设置与初始迁移版本。
- 向后兼容
  - ensureColumn 在旧库上按需添加缺失列，避免破坏已有数据。
  - 迁移函数处理历史数据修复（如助手消息片段压缩、遗留完成状态修复、本地 Qwen 预设修复或补全）。
- 幂等性与回滚
  - 迁移以事务包裹，失败则回滚。
  - 迁移仅在不存在的版本时执行，重复打开不重复应用。
  - 当前未提供"向下回滚"机制；如需回滚需手动维护反向迁移脚本并在部署流程中控制顺序。

**更新** 新增了第9版迁移，为 threads 表添加 workspace_id 和 pinned 字段，支持新的工作区绑定和线程固定功能。

```mermaid
flowchart TD
Start(["应用启动"]) --> CheckMig["检查 schema_migrations"]
CheckMig --> Apply{"是否已执行?"}
Apply -- 否 --> Exec["执行迁移任务"]
Exec --> Record["记录版本与时间"]
Record --> Next{"下一个迁移"}
Next -- 继续 --> Apply
Next -- 结束 --> Done(["就绪"])
Apply -- 是 --> Next
```

图表来源
- [src/agent/database.ts:1078-1090](file://src/agent/database.ts#L1078-L1090)
- [src/agent/database.ts:981-986](file://src/agent/database.ts#L981-L986)

章节来源
- [src/agent/database.ts:981-986](file://src/agent/database.ts#L981-L986)
- [src/agent/database.ts:1078-1090](file://src/agent/database.ts#L1078-L1090)
- [tests/database.test.ts:108-116](file://tests/database.test.ts#L108-L116)

### 事务与一致性
- 所有写操作均通过 transaction 包装，使用 BEGIN IMMEDIATE 提升并发安全性。
- 异常时自动 ROLLBACK，保证数据一致性。
- 典型场景：
  - 创建线程：单条插入。
  - 删除线程：级联标记删除。
  - 完成回合：更新 turns 并批量修正 items.incomplete。
  - 失败/中断：统一更新 turns 与 pending approvals。

**更新** 新增了线程固定和工作区绑定的事务操作，确保数据一致性。

```mermaid
sequenceDiagram
participant C as "调用方"
participant DB as "SqliteAppDatabase"
participant S as "SQLite"
C->>DB : completeTurn(turnId, completedAt, metrics?)
DB->>S : BEGIN IMMEDIATE
DB->>S : UPDATE turns SET status='completed'...
DB->>S : SELECT items WHERE turn_id=? AND kind IN(...)
DB->>S : UPDATE items SET payload=... (incomplete=false)
DB->>S : COMMIT
DB-->>C : boolean completed
```

图表来源
- [src/agent/database.ts:467-493](file://src/agent/database.ts#L467-L493)
- [src/agent/database.ts:191-253](file://src/agent/database.ts#L191-L253)

章节来源
- [src/agent/database.ts:467-493](file://src/agent/database.ts#L467-L493)
- [src/agent/database.ts:191-253](file://src/agent/database.ts#L191-L253)
- [tests/database.test.ts:485-511](file://tests/database.test.ts#L485-L511)

### 流式写入与合并
- appendItem 会识别逻辑流键（answer/reasoning），若存在同一流条目则追加文本而非新增行。
- 重启时会压缩历史助手消息片段，减少冗余。
- 该机制显著降低高吞吐流式写入时的行数膨胀。

```mermaid
flowchart TD
In(["收到 Item"]) --> Key["计算逻辑流键"]
Key --> Exists{"是否存在同流条目?"}
Exists -- 是 --> Merge["追加文本并更新 payload"]
Exists -- 否 --> Insert["INSERT 新行"]
Merge --> End(["完成"])
Insert --> End
```

图表来源
- [src/agent/database.ts:1042-1076](file://src/agent/database.ts#L1042-L1076)
- [src/agent/database.ts:1092-1127](file://src/agent/database.ts#L1092-L1127)

章节来源
- [src/agent/database.ts:1042-1076](file://src/agent/database.ts#L1042-L1076)
- [src/agent/database.ts:1092-1127](file://src/agent/database.ts#L1092-L1127)
- [tests/database.test.ts:360-371](file://tests/database.test.ts#L360-L371)
- [tests/database.test.ts:460-483](file://tests/database.test.ts#L460-L483)

### 数据模型与约束
- 实体关系
  - Thread -> Turns（一对多）
  - Thread -> Items（一对多）
  - Turn -> Items（一对多，可空）
  - Thread -> Approvals（一对多）
  - Turn -> Approvals（一对多）
  - ModelProfiles 独立，被 Threads 引用
  - Workspaces 独立，唯一 canonical_root_path
  - FileCheckpoints 关联 Workspaces 和 Turns

**更新** 移除了 ChatGroup 实体，Threads 现在直接关联 Workspaces，简化了数据模型。

- 验证与约束
  - 必填字段通过 requireTrimmed 校验。
  - 布尔/枚举类字段在映射与规范化函数中做归一化处理。
  - 外键约束启用，删除时级联或置空。
  - 设置项有默认值与范围限制（如 contextMessageLimit 上限 200）。

章节来源
- [src/shared/domain.ts:1-200](file://src/shared/domain.ts#L1-L200)
- [src/agent/database.ts:1389-1400](file://src/agent/database.ts#L1389-L1400)
- [src/agent/database.ts:995-1013](file://src/agent/database.ts#L995-L1013)

### 启动修复与一致性保障
- 重启后会将处于 queued/running/cancelling 的 turns 标记为 interrupted，并将相关 pending approvals 拒绝。
- 对历史遗留的不完整完成状态进行修复，避免误判成功。
- 对本地 Qwen 占位配置进行修复或补全，确保首次运行可用。

章节来源
- [src/agent/database.ts:495-517](file://src/agent/database.ts#L495-L517)
- [src/agent/database.ts:1234-1291](file://src/agent/database.ts#L1234-L1291)
- [tests/database.test.ts:373-428](file://tests/database.test.ts#L373-L428)

### 线程固定与工作区绑定
**新增功能** 数据库层现在支持线程固定和工作区绑定功能：

- **线程固定**：通过 setThreadPinned 方法可以标记重要线程，pinned 字段用于 UI 显示优先级。
- **工作区绑定**：通过 bindThreadWorkspace 方法可以将线程绑定到特定工作区，支持动态绑定和解绑。
- **创建时绑定**：createThread 方法现在接受可选的 workspaceId 参数，可以在创建线程时直接绑定工作区。

```mermaid
sequenceDiagram
participant U as "用户"
participant DB as "数据库"
U->>DB : createThread(title, workspaceId?)
DB->>DB : 验证工作区存在性
DB->>DB : 创建线程并设置 pinned=false
U->>DB : setThreadPinned(threadId, true)
DB->>DB : 更新线程的 pinned 状态
U->>DB : bindThreadWorkspace(threadId, workspaceId)
DB->>DB : 验证工作区并更新绑定关系
```

图表来源
- [src/agent/database.ts:255-291](file://src/agent/database.ts#L255-L291)
- [src/agent/database.ts:293-312](file://src/agent/database.ts#L293-L312)

章节来源
- [src/agent/database.ts:255-312](file://src/agent/database.ts#L255-L312)
- [tests/database.test.ts:720-759](file://tests/database.test.ts#L720-L759)

## 依赖关系分析
- 外部依赖
  - node:sqlite 提供的 DatabaseSync 同步访问 SQLite。
- 内部依赖
  - shared/domain.ts 定义的数据类型。
  - localQwenProfile.ts 提供本地 Qwen 预设输入。
  - modelCapabilities.ts 提供能力与推理设置的规范化。

```mermaid
graph LR
DB["database.ts"] --> DOM["shared/domain.ts"]
DB --> LQ["localQwenProfile.ts"]
DB --> MC["modelCapabilities.ts"]
DB --> SQLITE["node:sqlite (DatabaseSync)"]
```

图表来源
- [src/agent/database.ts:1-37](file://src/agent/database.ts#L1-L37)
- [src/agent/database.ts:1312-1314](file://src/agent/database.ts#L1312-L1314)

章节来源
- [src/agent/database.ts:1-37](file://src/agent/database.ts#L1-L37)
- [src/agent/database.ts:1312-1314](file://src/agent/database.ts#L1312-L1314)

## 性能与优化
- WAL 模式
  - 启用 PRAGMA journal_mode = WAL，提高并发读性能与崩溃恢复能力。
- 外键与忙等待
  - 启用 PRAGMA foreign_keys = ON 保证引用完整性。
  - PRAGMA busy_timeout = 5000 缓解短暂锁争用。
- 连接池
  - 当前使用单实例 DatabaseSync，未实现连接池。对于桌面端单进程场景通常足够；若未来扩展为多进程或多实例，需引入连接池或队列化访问。
- 事务粒度
  - 写操作均以事务包裹，减少磁盘落盘次数，提高吞吐。
- 批量操作
  - 完成回合时批量更新 items 的 incomplete 标志，避免多次往返。
- 流式合并
  - 通过逻辑流键合并文本，减少行数与 I/O。
- 索引建议
  - 针对高频查询列添加索引，如 threads.updated_at、turns.status、items.created_at、approvals.status 等，可显著提升列表与筛选性能。

**更新** 新增了线程固定和工作区绑定操作的优化，确保这些常用操作的响应速度。

章节来源
- [src/agent/database.ts:826-832](file://src/agent/database.ts#L826-L832)
- [src/agent/database.ts:467-493](file://src/agent/database.ts#L467-L493)
- [src/agent/database.ts:1042-1076](file://src/agent/database.ts#L1042-L1076)

## 故障排查指南
- 迁移失败
  - 检查 schema_migrations 是否记录了相应版本；确认迁移函数内 SQL 无语法错误。
  - 若迁移中途失败，事务会自动回滚，数据保持原状。
- 锁冲突
  - 出现忙锁时可适当增大 busy_timeout；避免长时间持有事务。
- 流式数据异常
  - 检查逻辑流键生成是否正确；确认同一流条目是否被正确合并。
- 状态不一致
  - 重启后检查 interrupted 与 pending approvals 是否被修复；必要时查看迁移 3/5 的行为。
- 模型配置问题
  - 检查 model_profiles 的 capabilities 与 reasoning 是否正常解析；确认默认 profile 是否已设置。
- **工作区绑定问题**
  - 如果 createThread 抛出工作区不存在错误，检查工作区是否已正确注册。
  - 如果 bindThreadWorkspace 失败，确认目标工作区存在且权限正确。

**更新** 新增了工作区绑定相关的故障排查指导。

章节来源
- [src/agent/database.ts:1078-1090](file://src/agent/database.ts#L1078-L1090)
- [src/agent/database.ts:495-517](file://src/agent/database.ts#L495-L517)
- [src/agent/database.ts:1234-1291](file://src/agent/database.ts#L1234-L1291)
- [tests/database.test.ts:430-458](file://tests/database.test.ts#L430-L458)

## 结论
该数据库层以 SQLite 为核心，采用简洁而稳健的设计：通过迁移系统保证版本演进与向后兼容，利用事务与流式合并确保一致性与性能，配合启动修复机制提升鲁棒性。**更新后的架构**移除了复杂的聊天组概念，简化了数据模型，同时增强了线程与工作区的绑定关系管理，提供了线程固定功能以支持重要的对话优先显示。建议在数据量增长后补充必要索引，并根据需要引入连接池与更细粒度的批处理策略。

## 附录：CRUD 与高级查询示例
以下示例以"步骤说明"形式给出，便于理解而不直接粘贴代码。

- **创建对话线程（支持工作区绑定）**
  - 调用 createThread(title, workspaceId?)，可选择性地绑定工作区。
  - 参考路径：[createThread:255-291](file://src/agent/database.ts#L255-L291)
- **设置线程固定状态**
  - 调用 setThreadPinned(threadId, pinned)，标记重要线程。
  - 参考路径：[setThreadPinned:293-300](file://src/agent/database.ts#L293-L300)
- **绑定线程到工作区**
  - 调用 bindThreadWorkspace(threadId, workspaceId)，将现有线程绑定到工作区。
  - 参考路径：[bindThreadWorkspace:302-312](file://src/agent/database.ts#L302-L312)
- **删除对话线程（软删除）**
  - 调用 deleteThread(threadId)。
  - 参考路径：[deleteThread:314-328](file://src/agent/database.ts#L314-L328)
- **设置线程使用的模型配置**
  - 调用 setThreadModel(threadId, modelProfileId?)，同时更新 activeModelProfileId。
  - 参考路径：[setThreadModel:330-340](file://src/agent/database.ts#L330-L340)
- **写入消息与推理项（流式合并）**
  - 调用 appendItem(item)，自动合并同一流文本。
  - 参考路径：[appendItem:519-543](file://src/agent/database.ts#L519-L543), [insertOrMergeItem:1042-1076](file://src/agent/database.ts#L1042-L1076)
- **提交审批**
  - 调用 upsertApproval(approval)，支持冲突更新。
  - 参考路径：[upsertApproval:545-570](file://src/agent/database.ts#L545-L570)
- **保存模型配置**
  - 调用 saveModelProfile(input)，规范化能力与推理设置，处理默认配置。
  - 参考路径：[saveModelProfile:572-652](file://src/agent/database.ts#L572-L652)
- **删除模型配置**
  - 调用 deleteModelProfile(id)，清理 threads 的引用并重置 activeModelProfileId。
  - 参考路径：[deleteModelProfile:654-661](file://src/agent/database.ts#L654-L661)
- **获取运行时模型配置**
  - 调用 getModelProfileForRuntime(id?)，优先选择 active/default。
  - 参考路径：[getModelProfileForRuntime:814-820](file://src/agent/database.ts#L814-L820)
- **注册工作区**
  - 调用 registerWorkspace(input)，按 canonical_root_path 去重。
  - 参考路径：[registerWorkspace:663-726](file://src/agent/database.ts#L663-L726)
- **获取/更新工作区**
  - 调用 getWorkspace(workspaceId)、touchWorkspace(workspaceId)。
  - 参考路径：[getWorkspace:736-741](file://src/agent/database.ts#L736-L741), [touchWorkspace:743-750](file://src/agent/database.ts#L743-L750)
- **获取快照（聚合视图）**
  - 调用 getSnapshot()，返回 threads、turns、items、approvals、modelProfiles、settings、workspaces。
  - 参考路径：[getSnapshot:191-253](file://src/agent/database.ts#L191-L253)
- **高级查询：按线程获取消息历史（合并助手回复）**
  - 调用 getThreadMessages(threadId, limit?)，合并同一 turn 的 assistant 文本。
  - 参考路径：[getThreadMessages:364-387](file://src/agent/database.ts#L364-L387)
- **高级查询：完成回合（原子更新 turns 与 items）**
  - 调用 completeTurn(turnId, completedAt, metrics?)，在事务中更新 turns 并修正 items.incomplete。
  - 参考路径：[completeTurn:467-493](file://src/agent/database.ts#L467-L493)

**更新** 移除了聊天组相关的 CRUD 操作，新增了线程固定和工作区绑定的操作方法。

章节来源
- [src/agent/database.ts:255-750](file://src/agent/database.ts#L255-L750)
- [tests/database.test.ts:720-759](file://tests/database.test.ts#L720-L759)
- [tests/database.test.ts:485-511](file://tests/database.test.ts#L485-L511)