...
- apply_patch 执行器：在内存中应用补丁并计算新内容哈希，支持创建/修改文件，生成 diff 行用于审批预览。
...
- 输出：返回 action、totalLines、linesChanged 与新 contentHash，供后续链式补丁使用。

```mermaid
flowchart TD
A["prepareApplyPatch(input, context)"] --> B["校验路径/大小/edits"]
B --> C{"文件是否存在?"}
C --> |是| D["读取原文本并计算 previousContentHash"]
D --> E{"baseContentHash 匹配?"}
E --> |否| Err["stale-content 错误"]
E --> |是| F["applyEdits() 生成 patched"]
C --> |否| G["创建新文件准备"]
G --> F
F --> H["recordFileChange() 记录检查点"]
H --> I{"是否新文件?"}
I --> |是| J["mkdirSync(父目录)"]
I --> |否| K["跳过"]
J --> L["writeFileSync(落盘)"]
K --> L
L --> M["返回 output(contentHash)"]
```
